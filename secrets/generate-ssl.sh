...
CN=`hostname -f`
LOCATION=$CITY
PASS=`cat $KEY_CREDENTIALS_LOCATION`
...
trust_store_private_key_file=""
  ...
  echo "Generating $CA_FILE from $DEFAULT_TRUSTSTORE_FILENAME"
  keytool -keystore $trust_store_file -export -alias CARoot -rfc -storepass $PASS -noprompt -file $TRUSTSTORE_WORKING_DIRECTORY/$CA_FILE
